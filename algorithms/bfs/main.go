// BFS bot — standalone checkers player using BFS game-tree search.
//
// Usage:
//   go run ./algorithms/bfs --api http://localhost:8080 --game <game-id> --player red
//
// The bot subscribes to the game's SSE /watch stream and moves whenever
// the board's "turn" matches its color.
package main
...
func main() {
	...
	playerStr := flag.String("player", "red", "Player color: red or black")
	flag.Parse()
	...
	log.Printf("BFS bot starting — game=%s player=%s", *gameID, *playerStr)
	watchSSE(*api, *gameID, color)
}

// watchSSE subscribes to the game's SSE stream and plays when it's our turn.
...
func handleBoard(api, gameID string, color engine.Color, raw string) {
	...
	// Reconstruct position from pieces
	pos := positionFromPieces(board.Pieces)

	// Think (brief delay to feel natural)
	...
	log.Printf("BFS moved: %d→%d", engine.InternalToACF(move.From), engine.InternalToACF(move.To))
}
